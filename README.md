...
提取 YouTube 视频字幕（简中/繁中/英），扫描其中 AI 常用句式与文本结构特征，加权计算后给出 **0–100 的"文案由 AI 生成"概率得分**，并在命中示例点击后跳转到视频对应时间。无字幕的视频不显示面板。默认不自动检测，点击浏览器工具栏扩展图标 → 弹窗 → "检测此视频"才触发，不干扰浏览。
...
- 点击浏览器工具栏扩展图标 → 弹窗 → "检测此视频"触发检测，弹窗内可开启"自动检测"（默认关）
- 检测结果以视频页右下角徽标显示得分，点击展开命中明细
...
4. 打开任意 YouTube 视频页面（`https://www.youtube.com/watch?v=...`）
5. 点击浏览器工具栏的 NoAIText-YT 图标 → 弹窗 → 点击"检测此视频"开始检测

无需登录即可读公开字幕；私密/会员视频需登录 YouTube 会话读取可见内容。

## 使用

- **触发检测**：点击浏览器工具栏扩展图标 → 弹窗 → "检测此视频"。弹窗中可开启"自动检测"，开启后每次打开视频自动检测
- **徽标配色**：绿 = 大概率真人（0–29），黄 = 疑似 AI 辅助（30–59），红 = 大概率 AI 文案（60–100）
- **面板**：总分、判定档、字幕字数/密度（中文显示「字」，英文显示「词」）、字幕轨切换、自动翻译开关、命中规则明细
- **无字幕**：自动检测时无字幕视频**不显示**面板（无文本可分析）；手动检测时显示错误提示
...
```
扩展图标弹窗 → "检测此视频" → content.js(sendMessage)
